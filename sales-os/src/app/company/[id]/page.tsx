...
import { getCompanyFromBackend, getFollowups } from '@/lib/backend-api';
...
export default async function CompanyDetailPage({ params }: { params: { id: string } }) {
  const [c, followups] = await Promise.all([
    getCompanyFromBackend(params.id),
  ...
  return (
    <div className="space-y-4 p-4 md:p-6 animate-fade-in">
      ...
      <FollowupTimeline companyId={c.id} followups={followups} />
    </div>
  );
}
